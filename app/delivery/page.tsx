    ...
    <div dir="rtl">
      ...
      <div className="mb-4 relative">
        ...
        <input
          ...
          className="w-full pr-10 pl-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-600/30 focus:border-brand-600"
        />
        {search && (
          <button
            onClick={() => setSearch('')}
            className="absolute left-3 top-1/2 -translate-y-1/2 p-1 rounded hover:bg-slate-100"
...
function EmptyColumn({ message }: { message: string }) {
  return (
    <div className="text-center py-10 text-xs text-slate-400 border-2 border-dashed border-slate-200 rounded-xl bg-white/40">
      {message}
    </div>
  );
}
    ...
    <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm">
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="min-w-0">
          ...
          <p className="text-sm font-semibold text-slate-800 truncate mt-0.5">{order.customerName}</p>
        </div>
      ...
      <div className="flex items-center gap-2 text-xs text-slate-600 mb-2">
        ...
        <span className="text-slate-500">{DELIVERY_TYPE_LABEL[order.deliveryType] ?? order.deliveryType}</span>
      </div>

      {/* Items summary */}
      <div className="bg-slate-50 rounded-lg p-2 mb-2 text-[11px]">
      ...
      <div className="flex gap-1.5">
        <a
          ...
          className="flex items-center justify-center gap-1 flex-1 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-[11px] font-medium rounded-lg transition-colors"
          title="اتصال"
        >
