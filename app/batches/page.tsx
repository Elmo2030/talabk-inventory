...
type BatchStatus = 'expired' | 'expiring_soon' | 'ok';
...
function DaysCell({ daysRemaining, status }: { daysRemaining: number; status: BatchStatus }) {
  ...
  return (
    <span className={`font-semibold text-sm ${status === 'expiring_soon' ? 'text-amber-700' : 'text-green-700'}`}>
...
export default function BatchesPage() {
  ...
  const { expired, expiringSoon, ok } = useMemo(() => {
    ...
    return {
      expired: batches.filter((b) => b.status === 'expired').sort((a, b) => a.daysRemaining - b.daysRemaining),
      expiringSoon: batches.filter((b) => b.status === 'expiring_soon').sort((a, b) => a.daysRemaining - b.daysRemaining),
      ok: batches.filter((b) => b.status === 'ok').sort((a, b) => a.daysRemaining - b.daysRemaining),
    };
  }, [stockIn, items]);

  const total = expired.length + expiringSoon.length + ok.length;
    ...
    <div dir="rtl">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-red-600 font-medium">منتهية الصلاحية</p>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-amber-600 font-medium">تنتهي خلال 30 يوم</p>
      ...
      {total === 0 ? (
        ...
        <div className="space-y-6">
          ...
          {expiringSoon.length > 0 && <SectionTable rows={expiringSoon} status="expiring_soon" />}
